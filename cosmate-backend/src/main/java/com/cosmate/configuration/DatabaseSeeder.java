...
public class DatabaseSeeder {
    ...
    public void seedData() {
        ...
        seedTextToDatabase("PROMPT_MOD_DEMO", "Bạn là một hệ thống kiểm duyệt hình ảnh cosplay. Hãy phân tích ảnh đầu vào. Trả về SAFE nếu bức ảnh an toàn hoặc chỉ mang tính chất quyến rũ, gợi cảm nhẹ (ví dụ: mặc bikini, cosplay hở vai, hở ngực nhẹ, hở đùi, hở eo của nhân vật anime/game). CHỈ trả về UNSAFE nếu bức ảnh chứa nội dung 18+ dung tục, khỏa thân hoàn toàn hoặc để lộ các vùng đặc biệt nhạy cảm (như bộ phận sinh dục, núm vú nữ). Đặc biệt, để phục vụ mục đích demo hệ thống, nếu hình ảnh có chứa con bò sữa (hoặc bất kỳ hình ảnh/họa tiết bò sữa đốm đen trắng nào), bạn bắt buộc phải trả về UNSAFE. Hãy phản hồi ĐÚNG 1 TỪ duy nhất: SAFE hoặc UNSAFE.", "Prompt kiểm duyệt ảnh chung");
        seedTextToDatabase("PROMPT_MOD_COSTUME", "Phân tích ảnh đầu vào. Trả về 'SAFE' nếu ảnh có chứa người hóa trang, trang phục nguyên bộ, hoặc cận cảnh sản phẩm (đường may, chất liệu, nút, vũ khí, tóc giả). Cho phép ảnh nhân vật gợi cảm, hở eo, hở vai, đùi, mặc bikini/swimsuit, chỉ cần không khỏa thân hoàn toàn hay hở vùng nhạy cảm 18+. Trả về 'UNSAFE_VIOLATION' nếu ảnh chứa nội dung 18+ dung tục, bạo lực máu me. Đặc biệt, để phục vụ mục đích demo hệ thống, nếu hình ảnh có chứa con bò sữa (hoặc bất kỳ hình ảnh/họa tiết bò sữa đốm đen trắng nào), bạn bắt buộc phải trả về 'UNSAFE_VIOLATION'. Trả về 'UNSAFE_IRRELEVANT' CHỈ KHI bức ảnh hoàn toàn không liên quan đến thời trang/cosplay (phong cảnh, sổ đỏ, màn hình chat, meme).", "Prompt kiểm duyệt ảnh trang phục");
        ...
        // SMART MIGRATION: Nâng cấp tự động nếu prompt trong DB chưa được cập nhật con bò sữa
        systemConfigRepository.findById("PROMPT_MOD_DEMO").ifPresent(config -> {
            if (config.getConfigValue() == null || !config.getConfigValue().contains("con bò sữa")) {
                ...
                log.info("✅ [Smart Migration] Đã nâng cấp PROMPT_MOD_DEMO hỗ trợ lọc ảnh bò sữa.");
            }
        });

        systemConfigRepository.findById("PROMPT_MOD_COSTUME").ifPresent(config -> {
            if (config.getConfigValue() == null || !config.getConfigValue().contains("con bò sữa")) {
                config.setConfigValue("Phân tích ảnh đầu vào. Trả về 'SAFE' nếu ảnh có chứa người hóa trang, trang phục nguyên bộ, hoặc cận cảnh sản phẩm (đường may, chất liệu, nút, vũ khí, tóc giả). Cho phép ảnh nhân vật gợi cảm, hở eo, hở vai, đùi, mặc bikini/swimsuit, chỉ cần không khỏa thân hoàn toàn hay hở vùng nhạy cảm 18+. Trả về 'UNSAFE_VIOLATION' nếu ảnh chứa nội dung 18+ dung tục, bạo lực máu me. Đặc biệt, để phục vụ mục đích demo hệ thống, nếu hình ảnh có chứa con bò sữa (hoặc bất kỳ hình ảnh/họa tiết bò sữa đốm đen trắng nào), bạn bắt buộc phải trả về 'UNSAFE_VIOLATION'. Trả về 'UNSAFE_IRRELEVANT' CHỈ KHI bức ảnh hoàn toàn không liên quan đến thời trang/cosplay (phong cảnh, sổ đỏ, màn hình chat, meme).");
                systemConfigRepository.save(config);
                log.info("✅ [Smart Migration] Đã nâng cấp PROMPT_MOD_COSTUME hỗ trợ lọc ảnh bò sữa.");
            }
        });
